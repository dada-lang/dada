...
pub(crate) async fn term_is_ktb_owned<'db>(
    env: &Env<'db>,
    term: SymGenericTerm<'db>,
) -> Errors<bool> {
    match term {
        SymGenericTerm::Type(sym_ty) => ty_is_ktb_owned(env, sym_ty).await,
        SymGenericTerm::Perm(sym_perm) => perm_is_ktb_owned(env, sym_perm).await,
...
#[boxed_async_fn]
async fn ty_is_ktb_owned<'db>(env: &Env<'db>, ty: SymTy<'db>) -> Errors<bool> {
    let db = env.db();
    match *ty.kind(db) {
        SymTyKind::Perm(sym_perm, sym_ty) => {
            Ok(application_is_ktb_owned(env, sym_perm.into(), sym_ty.into()).await?)
        }
        SymTyKind::Infer(infer) => {
            Ok(test_infer_is_known_to_be(env, infer, Predicate::Owned).await)
        }
        SymTyKind::Var(var) => Ok(test_var_is_known_to_be(env, var, Predicate::Owned)),
            ...
            SymTyName::Aggregate(sym_aggregate) => match sym_aggregate.style(db) {
                SymAggregateStyle::Struct => {
                    for_all(generics, async |&generic| {
                        term_is_ktb_owned(env, generic).await
                    })
                    .await
                }
...
async fn application_is_ktb_owned<'db>(
    env: &Env<'db>,
    lhs: SymGenericTerm<'db>,
    rhs: SymGenericTerm<'db>,
) -> Errors<bool> {
    both(term_is_ktb_owned(env, lhs), term_is_ktb_owned(env, rhs)).await
}

#[boxed_async_fn]
pub(crate) async fn perm_is_ktb_owned<'db>(env: &Env<'db>, perm: SymPerm<'db>) -> Errors<bool> {
    let db = env.db();
    match *perm.kind(db) {
        ...
        SymPermKind::Leased(ref places) => {
            exists(places, async |&place| place_is_ktb_owned(env, place).await).await
        }

        SymPermKind::Apply(lhs, rhs) => {
            Ok(application_is_ktb_owned(env, lhs.into(), rhs.into()).await?)
        }

        SymPermKind::Var(var) => Ok(test_var_is_known_to_be(env, var, Predicate::Owned)),
...
pub(crate) async fn place_is_ktb_owned<'db>(env: &Env<'db>, place: SymPlace<'db>) -> Errors<bool> {
    let ty = place.place_ty(env).await;
    ty_is_ktb_owned(env, ty).await
}
