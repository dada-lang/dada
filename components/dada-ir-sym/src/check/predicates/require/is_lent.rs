...
use crate::{
    check::{
        ...
        predicates::{
            ...
            test::{test_place_is_lent, test_place_is_move, test_term_is_lent, test_term_is_move},
...
async fn require_application_is_lent<'db>(
    ...
    require(
        either(
            test_term_is_lent(env, rhs),
            both(test_term_is_move(env, rhs), test_term_is_lent(env, lhs)),
        ),
            ...
            SymTyName::Tuple { arity } => {
                ...
                require(
                    exists(generics, async |&generic| {
                        test_term_is_lent(env, generic).await
                    }),
                    || report_term_must_be_but_isnt(env, span, term, Predicate::Lent),
                )
                .await
