...
fn require_red_perm_sub_red_perm<'db>(
    ...
    let db = env.db();
    if let Some(unmatched_chain) = lower_perm.chains(db).iter().find(|lower_chain| {
        upper_perm
            .chains(db)
            .iter()
            .all(|upper_chain| !test_red_chain_sub_red_chain(env, lower_chain, upper_chain))
    }) {}
    Ok(())
}
...
async fn require_lower_chain<'db>(
    ...
    let Some(or_else) =
        env.insert_red_perm_bound(upper_head, &lower_chain, Direction::FromBelow, or_else)
    else {
        return Ok(());
    };
...
async fn require_upper_chain<'db>(
    ...
    let Some(_or_else) =
        env.insert_red_perm_bound(lower_head, &upper_chain, Direction::FromAbove, or_else)
    else {
        return Ok(());
    };
